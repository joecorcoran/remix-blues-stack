...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create workspace-f75f
  fly apps create workspace-f75f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app workspace-f75f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app workspace-f75f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'workspace-f75f-staging' does not match app name in config file 'workspace-f75f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name workspace-f75f-db
  fly postgres attach --app workspace-f75f workspace-f75f-db

  fly postgres create --name workspace-f75f-staging-db
  fly postgres attach --app workspace-f75f-staging workspace-f75f-staging-db
  ```
